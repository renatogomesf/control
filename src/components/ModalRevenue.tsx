...
export default function ModalRevenue({
  ...
  useLayoutEffect(() => {
    if (revenueToModal !== null) {
      dateRef.current!.value = revenueToModal.date;
      descriptionRef.current!.value = revenueToModal.description;
      valueRef.current!.value = revenueToModal.value;

      console.log("data" + revenueToModal.date);
    } else {
        ...
        <div className="bg-PRIMARY p-5 rounded-lg border border-QUATERNARY w-[300px]">
          ...
          {title === "Criar nova receita" ? (
            ...
            <Button
              className="mt-6"
              value={button}
              onClick={() => update(revenueToModal.idRevenue)}
            />
          ) : (
            ""
          )}
